...
void parAdvectOverlap(int reps, double *u, int ldu)
{
  ...
  for (r = 0; r < reps; r++)
  {
    ...
    // Update the inner halo first
    updateAdvectField(1, N_loc, &V(u, 1, 1), ldu, &V(v, 1, 1), ldv);
    updateAdvectField(M_loc, 1, &V(u, 1, 1), ldu, &V(v, 1, 1), ldv);

    // Initiate communication
    updateBoundary(v, ldv);

    // Update the rest of the data
    updateAdvectField(2, N_loc - 1, &V(u, 2, 1), ldu, &V(v, 2, 1), ldv);
    updateAdvectField(M_loc - 1, N_loc - 1, &V(u, 2, 1), ldu, &V(v, 2, 1), ldv);

    // Wait for communication to complete and copy the updated field to u
    copyField(M_loc, N_loc, &V(v, 1, 1), ldv, &V(u, 1, 1), ldu);
