...
static void updateBoundary(double *u, int ldu)
{
  ...
  else
  {
    ...
    MPI_Isend(&V(u, 1, 1), N_loc, MPI_DOUBLE, topProc, HALO_TAG, comm, &req[1]);
    MPI_Irecv(&V(u, M_loc + 1, 1), N_loc, MPI_DOUBLE, botProc, HALO_TAG, comm, &req[2]);
  ...
  else
  {
    ...
    MPI_Isend(&V(u, 0, 1), 1, column_type, leftProc, HALO_TAG, comm, &req[1]);
    MPI_Irecv(&V(u, 0, N_loc + 1), 1, column_type, rightProc, HALO_TAG, comm, &req[2]);
...
void parAdvectOverlap(int reps, double *u, int ldu)
{
  ...
  for (r = 0; r < reps; r++)
  {
    ...
    // star wait for boundary msg to arrive
    MPI_Waitall(4, req, MPI_STATUS_IGNORE);
    // left and right sides of halo
